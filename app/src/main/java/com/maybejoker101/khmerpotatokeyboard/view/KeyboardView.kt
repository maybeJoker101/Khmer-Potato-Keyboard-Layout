package com.maybejoker101.khmerpotatokeyboard.view
...
import com.maybejoker101.khmerpotatokeyboard.data.KeyboardData
import com.maybejoker101.khmerpotatokeyboard.data.Trie
...
import com.maybejoker101.khmerpotatokeyboard.khmer.KhmerLang
import com.maybejoker101.khmerpotatokeyboard.khmer.KhmerWord
import com.maybejoker101.khmerpotatokeyboard.util.InputTypeUtils
import com.maybejoker101.khmerpotatokeyboard.util.Util
import com.maybejoker101.khmerpotatokeyboard.view.button.*
import com.maybejoker101.khmerpotatokeyboard.view.list.WordListAdapter
...
class KeyboardView(context: Context, val inputService: InputMethodService) : androidx.constraintlayout.widget.ConstraintLayout(context) {
    ...
    val popupMiddleColor: Int?
    val popupOtherColor: Int?
...
//            topPlaceholder.setBackgroundColor(context.getColor(R.color.background_material_dark))
    ...
    fun buildPossibleWordRecycleView() {
        ...
        possibleWordRecycleView.setBackgroundColor(Color.parseColor(viewConfig.buttonBackgroundColor))
    ...
    fun buildComposingWordRecycleView() {
        ...
        composingWordRecycleView.setBackgroundColor(Color.parseColor(viewConfig.buttonBackgroundColor))
...
//        inputService.currentInputConnection.sendKeyEvent(KeyEvent(ACTION_UP, KEYCODE_DEL))
        ...
        if (!selectedText.isNullOrEmpty()) {
            ...
            Log.d(DEBUG_TAG, "textbeforecursor $textBeforeCursor")




            if (textBeforeCursor.isNotEmpty() && textBeforeCursor.last().toString() == ZERO_WIDTH_SPACE) {
                //if zero space  and only 2 characters
                ...
                Log.d(DEBUG_TAG, "startindex = $startIndex $maxIndex")
                ...
                if (startIndex == -1) {
                    ...
                    currentInput.deleteSurroundingText(2, 0)
                    Log.d(DEBUG_TAG, "startindex = $startIndex")
    ...
    fun setupPopupWindow() {
        //parent
        val popupParent = popupView.findViewById<ConstraintLayout>(R.id.popUpParentLayout)
        popupParent.setBackgroundColor(Color.parseColor(viewConfig.popupBackGroundColor))
        popupParent.layoutParams.width = Util.getPixelFromDp(viewConfig.popupHeightAndWidth, context)
        popupParent.layoutParams.height = Util.getPixelFromDp(viewConfig.popupHeightAndWidth, context)

        //Color
        val middleBig = popupView.findViewById<TextView>(R.id.popup_middle_big)
        val popupMiddlePrimary = popupView.findViewById<TextView>(R.id.popup_middle_primary)
        val popupTop = popupView.findViewById<TextView>(R.id.popup_top)
        val popupRight = popupView.findViewById<TextView>(R.id.popup_right)
